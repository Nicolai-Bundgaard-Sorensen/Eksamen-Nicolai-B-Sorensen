      ...
      <div className="my-page-list">
        ...
        {tab === "favorites" &&
          visibleFavorites.map((favorite) => (
            <article
              className={`result-card ${expandedJobId === favorite.jobListing.id ? "expanded" : ""}`}
              key={favorite.id}
            >
              <div className="result-card-main">
                <p className="result-company">
                  {favorite.jobListing.organization}
                </p>
                <h2>{favorite.jobListing.title}</h2>
                {expandedJobId === favorite.jobListing.id && (
                  <p className="result-category">
                    {favorite.jobListing.jobCategory?.name ?? "Kategori"}
                  </p>
                )}
                <p className="result-description">
                  {favorite.jobListing.description}
                </p>
                {expandedJobId === favorite.jobListing.id && (
                  <div className="result-expanded-details">
                    <div>
              ...
              <div className="result-card-side">
                <p>
                  Lokation:{" "}
                  {favorite.jobListing.region?.name ?? favorite.jobListing.city}
                </p>
                ...
                {expandedJobId === favorite.jobListing.id && (
                  <>
                    <p>
                      Arbejdstid:{" "}
                      {favorite.jobListing.workType?.type ?? "Ikke oplyst"}
                    </p>
                    <p>
                      Hjemmearbejde:{" "}
                      {favorite.jobListing.workHome ?? "Ikke oplyst"}
                    </p>
                  </>
                )}
                <div className="result-actions">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setExpandedJobId((id) =>
                        id === favorite.jobListing.id
                          ? null
                          : favorite.jobListing.id,
                      )
                    }
                  >
        ...
        {tab === "listings" && !ownJobs.length && (
          <p className="my-page-empty">Du har ingen annoncer endnu.</p>
        )}
        {tab === "favorites" && !favorites.length && (
          <p className="my-page-empty">Du har ingen favoritter endnu.</p>
        )}
